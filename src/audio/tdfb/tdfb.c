...
#define CTRL_INDEX_DIRECTION		0
#define CTRL_INDEX_FILTERBANK		0
...
static void *tdfb_filter_seek(struct sof_tdfb_config *config, int num_filters)
{
	...
	int i;
	uint8_t *coefp = (uint8_t *)ASSUME_ALIGNED(&config->data[0], 2); /* 2 for 16 bits data */
...
static int tdfb_cmd_switch_get(struct sof_ipc_ctrl_data *cdata, struct tdfb_comp_data *cd)
{
	...
	if (cdata->index != CTRL_INDEX_ONOFF)
		return -EINVAL;
...
static int tdfb_cmd_enum_get(struct sof_ipc_ctrl_data *cdata, struct tdfb_comp_data *cd)
{
	...
	/* Fail if wrong index in control, needed if several in same type */
	if (cdata->index != CTRL_INDEX_DIRECTION)
		return -EINVAL;
...
static int tdfb_cmd_get_value(struct comp_dev *dev, struct sof_ipc_ctrl_data *cdata)
{
	...
	case SOF_CTRL_CMD_SWITCH:
		...
		ret = tdfb_cmd_switch_get(cdata, cd);
		break;
	default:
...
{
	if (cdata->index != CTRL_INDEX_DIRECTION || cdata->num_elems != 1)
		return -EINVAL;
...
{
	if (cdata->index != CTRL_INDEX_ONOFF || cdata->num_elems != 1)
		return -EINVAL;
...
static int tdfb_cmd_set_value(struct comp_dev *dev, struct sof_ipc_ctrl_data *cdata)
{
	...
	case SOF_CTRL_CMD_SWITCH:
		...
		ret = tdfb_cmd_switch_set(cdata, cd);
		break;
	default:
